...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ssma: subbytes, shiftrows, mixcolumn, addroundkey
void ssma(__m128i *state, int round)
{
    uint8 *s = (uint8 *)state;
    __m128i R = _mm_setr_epi32(S_01[s[0]] ^ S_02[s[5]] ^ S_03[s[10]] ^ S_04[s[15]],
                               S_01[s[4]] ^ S_02[s[9]] ^ S_03[s[14]] ^ S_04[s[3]],
                               S_01[s[8]] ^ S_02[s[13]] ^ S_03[s[2]] ^ S_04[s[7]],
                               S_01[s[12]] ^ S_02[s[1]] ^ S_03[s[6]] ^ S_04[s[11]]);
    *state = _mm_xor_si128(R, KEY[round]);
    return;
}

// ssa: subbtyes, shiftrows, addroundkey
void ssa(__m128i *state)
{
    uint8 *s = (uint8 *)state;
    __m128i R = _mm_setr_epi8(S[s[0]], S[s[5]], S[s[10]], S[s[15]],
                              S[s[4]], S[s[9]], S[s[14]], S[s[3]],
                              S[s[8]], S[s[13]], S[s[2]], S[s[7]],
                              S[s[12]], S[s[1]], S[s[6]], S[s[11]]);
...
void keygen(uint32 *key)
{
    ...
    {
        if (!(i & 3)) //if i%4==0
            w[i] = (S[w[i - 1] >> 24] | (S[(w[i - 1] >> 16) & 0xff] << 24) |
                    (S[(w[i - 1] >> 8) & 0xff] << 16) | (S[w[i - 1] & 0xff] << 8)) ^
                   RC[i / 4 - 1] ^ w[i - 4];
        else
        {
            w[i] = w[i - 1] ^ w[i - 4];
            if (i & 1 & (i >> 1)) //if i%4==3
                KEY[i >> 2] = _mm_set_epi32(w[i - 3], w[i - 2], w[i - 1], w[i]);
        }
    }
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        for (int j = 0; j < 16; j++)
            t[j] = s[15 - j];
        KEY[i] = _mm_loadu_si128((__m128i *)t);
    }
    return;
}
...
void InvMix(uint8 *s)
{
    ...
    {
        a = 15 - (i << 2), b = 14 - (i << 2), c = 13 - (i << 2), d = 12 - (i << 2);
        tmp0 = s[a], tmp1 = s[b], tmp2 = s[c], tmp3 = s[d];
        s[a] = gfmul(tmp0, 14) ^ gfmul(tmp1, 11) ^
               gfmul(tmp2, 13) ^ gfmul(tmp3, 9);
        s[b] = gfmul(tmp0, 9) ^ gfmul(tmp1, 14) ^
               gfmul(tmp2, 11) ^ gfmul(tmp3, 13);
        s[c] = gfmul(tmp0, 13) ^ gfmul(tmp1, 9) ^
               gfmul(tmp2, 14) ^ gfmul(tmp3, 11);
        s[d] = gfmul(tmp0, 11) ^ gfmul(tmp1, 13) ^
               gfmul(tmp2, 9) ^ gfmul(tmp3, 14);
    }
}

// in inv_keygen, key's inv_mix operations are done
void inv_keygen(uint32 *key)
{
    uint32 w[44] = {0};
    uint8 t[16] = {0};
    w[0] = key[0], w[1] = key[1], w[2] = key[2], w[3] = key[3];
    KEY[10] = _mm_set_epi32(w[0], w[1], w[2], w[3]);
    for (int i = 4; i < 44; i++)
    {
        if (!(i & 3)) //if i%4==0
            w[i] = (S[w[i - 1] >> 24] | (S[(w[i - 1] >> 16) & 0xff] << 24) |
        ...
        else
        {
            w[i] = w[i - 1] ^ w[i - 4];
            if (i & 1 & (i >> 1)) //if i%4==3
                KEY[10 - (i >> 2)] = _mm_set_epi32(w[i - 3], w[i - 2], w[i - 1], w[i]);
        }
    }
    uint8 *k;
    uint8 tmp[16] = {0};
    // do inv_mix to 1~9 key
    for (int i = 1; i < 10; i++)
    {
        k = (uint8 *)&KEY[i];
        InvMix(k);
        KEY[i] = _mm_loadu_si128((__m128i *)k);
    }
    for (int i = 0; i < 11; i++)
    {
        uint8 *s = (uint8 *)&KEY[i];
...
// inv_ssma: invsubbytes, invshiftrows, invmixcolumns, addroundkey(the key has been done inv_mix)
void inv_ssma(__m128i *state, int round)
{
    uint8 *s = (uint8 *)state;
    __m128i R = _mm_setr_epi32(S_I_01[s[0]] ^ S_I_02[s[13]] ^ S_I_03[s[10]] ^ S_I_04[s[7]],
                               S_I_01[s[4]] ^ S_I_02[s[1]] ^ S_I_03[s[14]] ^ S_I_04[s[11]],
                               S_I_01[s[8]] ^ S_I_02[s[5]] ^ S_I_03[s[2]] ^ S_I_04[s[15]],
                               S_I_01[s[12]] ^ S_I_02[s[9]] ^ S_I_03[s[6]] ^ S_I_04[s[3]]);
    *state = _mm_xor_si128(R, KEY[round]);
    return;
}

// inv_ssa: invsubbytes, invshiftrows, addroundkey
void inv_ssa(__m128i *state)
{
    uint8 *s = (uint8 *)state;
    __m128i R = _mm_setr_epi8(S_I[s[0]], S_I[s[13]], S_I[s[10]], S_I[s[7]],
                              S_I[s[4]], S_I[s[1]], S_I[s[14]], S_I[s[11]],
                              S_I[s[8]], S_I[s[5]], S_I[s[2]], S_I[s[15]],
                              S_I[s[12]], S_I[s[9]], S_I[s[6]], S_I[s[3]]);
    *state = _mm_xor_si128(R, KEY[10]);
    return;
}

__m128i decrypt(uint8 *c)
{
    __m128i state = _mm_loadu_si128((__m128i *)c);
    state = _mm_xor_si128(state, KEY[0]);
    inv_ssma(&state, 1);
    inv_ssma(&state, 2);
    inv_ssma(&state, 3);
...
int main()
{
    ...
    for (int i = 0; i < 100000000; i++)
        state = encrypt(m); // step(2)
    end = clock();
    printf("encrypt 10^8 times: %lf s\n", (double)(end - start) / CLOCKS_PER_SEC);
    //output ciphertext
    uint8 *r = (uint8 *)&state;
    for (j = 0; j < 16; j++)
        printf("%02x ", r[j]);
    ...
    start = clock();
    for (int i = 0; i < 100000000; i++)
        state = decrypt(c); // step(2)
    end = clock();
    printf("decrypt 10^8 times: %lf s\n", (double)(end - start) / CLOCKS_PER_SEC);
    //output message
    uint8 *t = (uint8 *)&state;
    for (j = 0; j < 16; j++)
        printf("%02x ", t[j]);
